...
1. `representation_sampling`: The main modeling script that `server_run.sh` runs for each run for each replica. Note that this script expects the input data to be in a different directory structure than the `input` directory in this repository. The expected structure is as follows: all the required fasta files and the PDB files inside a directory called `data` in the working directory (from which the script is called), with a subdirectory `data/gmm` that contains the `.mrc` files and the GMM `.txt` files (see README in `data/em`)
...
1. `extract_all.sh`: The set of commands to reproduce the analysis results. This requires all the "output" folders, corresponding to the 45 runs sampled above, to be present in the directory this script is run from. Output directories have to be created manually.
...
The best predicted PDB for each protein-pair as well as the input sequences are found in protein-pair specific directories in `analysis/alphafold`. The script `alphafold_analyze.py` generates the pLDDT/PAE figures (not used in paper, needs the `.pkl` files) and the interface lists. Generated PAE/PLDDT figures and the contact lists are uploaded (`alphafold/analysis_output`) without the original `.pkl` files due to space constraints.
