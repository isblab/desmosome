...
if __name__ == '__main__':
    ...
    npkp = len(p['PKP3a']) // 4
    npg = len(p['PG']) // 4
    restraint_domains = [('PKP3a', [i for i in range(17)]), ('PKP3a', [i for i in range(17, npkp)]),
    ...
    # Binding restraints
    domain1 = [('PKP3a', 13, 2), ('PKP3a', 13, 2), ('PKP3a', 3, 0), ('PKP3a', 13, 2), ('PG', 31, 0), ('DP', 9, 0)]
    domain2 = [('DSG1', 14, 0), ('DSC1', 9, 0), ('DP', 4, 0), ('DP', 23, 0), ('DP', 23, 0), ('DSC1', 9, 0)]
    restraint_names = ['PKP-DSG', 'PKP-DSC', 'PKP-DP-1', 'PKP-DP-2', 'PG-DP', 'DP-DSC']
    n_copies = {'PKP3a': 4, 'DP': 4, 'PG': 4, 'DSG1': 2, 'DSC1': 2}
    all_ds = []
    for i, name in tqdm.tqdm(enumerate(restraint_names), desc='Binding Restraints'):
        ...
        if i in [0, 1, 3]:  # discontinuous binding domains for PKP3a
            protein_copies1 = [np.concatenate([[0], np.arange(offset1, offset1 + len1)]) + i * n_d1
                               for i in range(n_copies[d1])]
