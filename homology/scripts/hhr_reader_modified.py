...
# Modified (and fixed) on 14-Dec-2020 by Satwik Pasani: stochastic13
...
def read_main(hhr_path, selected_template):
    ...
    for result in read_result(hhr_path):
        output2 = []
        if result.template_id.lower() == selected_template.lower():
            output2.append(result.template_ali)
            output2.append(result.query_ali)
            output2.append((result.start[1], result.end[1]))
